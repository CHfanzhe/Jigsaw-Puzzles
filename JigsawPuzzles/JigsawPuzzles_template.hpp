...
namespace JigsawPuzzles::Template {
    ...
    private:
        template<typename, typename> friend struct get_nametag;
...
//--->  Tool Type
    ...
    struct get_nametag { using type = void; };
    template<typename T>
    struct get_nametag<JigsawPuzzle<T>> { using type = T; };
    template<typename T>
    struct get_nametag<T, std::void_t<typename T::Tag>> {
        using type = std::conditional_t<std::is_base_of_v<JigsawPuzzle<typename T::Tag>, T>, 
                        typename T::Tag, void>;
    };
    template<typename T> using get_nametag_t = typename get_nametag<T>::type;
...
//--->  JigsawPuzzleBoard
    ...
    public:
        ...
        constexpr auto Load() noexcept 
            ...
            static_assert((std::tuple_size_v<result> == 1),
                "Load Faild: Find Zero Or More Same NameTag.");
